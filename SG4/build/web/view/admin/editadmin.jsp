                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="${pageContext.request.contextPath}/admin/admin/edit" method="post"> 
                            ...
                            <div class="form-group">
                                <label for="input-1">Chi nhánh</label>
                                <select class="form-control valid" id="input-6" name="department">
                                    <c:forEach items="${departmentlist}" var="i">
                                        <c:choose>
                                            <c:when test="${i.id == admin.department_id}">
                                                <option value="${i.id}" selected="selected">${i.describe}</option>
                                            </c:when>
                                            <c:otherwise>
                                                <option value="${i.id}">${i.describe}</option>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                </select>
